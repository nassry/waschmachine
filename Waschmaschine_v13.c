...
{
	lcd_curs (63);									// Cursor auf Position setzen					
	text_loeschen(63, 76);					// LCD-Text Löschen
}	
void text_loeschen(unsigned int cours_start, unsigned int cours_end)		// Funktion text loeschen 
{
	for (i = cours_start; i < cours_end; i++)															// For Schleife zählt Variable i hoch
	{
...
void waschgang(void)							// Funktion Waschgang
{
	...
	switch (zustand)								// Switch_Case Funktion
	{
		...
		case 3:												// Case 3; Waschen und Temperatur regeln für 60 Sekunden aktivieren
			if(zeitabfrage(60))					// Wenn zeitabfrage(60); Funktion Zeitabfrage gibt die tatsächliche Laufzeit von 60 Sekunden an
			{	
...
{
	ET0 = 0;																	// Interrupt Freigabe löschen
